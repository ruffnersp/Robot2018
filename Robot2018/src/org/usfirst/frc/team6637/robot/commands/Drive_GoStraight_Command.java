...
public class Drive_GoStraight_Command extends Command {
	...
	// encoders aren't resetting. store the initial distance, subtract from
	// ongoing average distance to get distanceSoFar
	...
	protected void execute() {
		...
		// drive normal, drive braked, drive in reverse braked
		Timer.delay(0.004);
		if (Robot.driveEncoderSubsystem.getAverageDistance() < inches - rampDown) {
			Robot.driveSubsystem.autonDrive(-power, angle * Kp, false);
		} else if (Robot.driveEncoderSubsystem.getAverageDistance() > (inches - rampDown)
				&& Robot.driveEncoderSubsystem.getAverageDistance() < inches) {
			Robot.driveSubsystem.autonDrive(-brakePower, angle * Kp, false);
		} else if (Robot.driveEncoderSubsystem.getAverageDistance() > inches) {
			Robot.driveSubsystem.autonDrive(brakePower, -angle * Kp, false);
		}

		// if the robot is within 2 inches of goal, start tolerance counter,
		// determines isFinished result
		if ((Robot.driveEncoderSubsystem.getAverageDistance() + inches) / 2 - inches < 2
				&& Robot.driveEncoderSubsystem.getAverageDistance() > inches / 2) {
			toleranceCount++;
		} else {
			toleranceCount = 0;
		}
